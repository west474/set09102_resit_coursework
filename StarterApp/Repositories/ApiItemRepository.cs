...
public class ApiItemRepository : ApiRepositoryBase, IItemRepository
{
    private const string ItemsEndPoint = "items";

    public ApiItemRepository(HttpClient httpClient) : base(httpClient){}
    ...
    {
        var response = await HttpClient.GetAsync(ItemsEndPoint);
    ...
    {
        var response = await HttpClient.GetAsync($"{ItemsEndPoint}/{id}");
    ...
    {
        var response = await HttpClient.PostAsJsonAsync(ItemsEndPoint, entity);
        response.EnsureSuccessStatusCode();
    ...
    {
        var response = await HttpClient.PutAsJsonAsync($"{ItemsEndPoint}/{entity.Id}", entity);
